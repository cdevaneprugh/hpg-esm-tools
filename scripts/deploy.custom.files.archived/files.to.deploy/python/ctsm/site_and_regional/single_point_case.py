...
class SinglePointCase(BaseCase):
    ...
    def create_tag(self):
        ...
        if self.site_name:
            ...
            self.tag = "{}_{}".format(self.plon.get(360), str(self.plat))
    ...
    def create_domain_at_point(self, indir, file):
        ...
        """
        logger.info("----------------------------------------------------------------------")
        logger.info("Creating domain file at %s, %s.", self.plon.get(360), str(self.plat))
        ...
        f_out = f_in.sel(ni=self.plon.get(360), nj=self.plat, method="nearest")
    ...
    def create_landuse_at_point(self, indir, file, user_mods_dir):
        ...
        # extract gridcell closest to plon/plat
        f_out = f_in.sel(lsmlon=self.plon.get(360), lsmlat=self.plat, method="nearest")
    ...
    def create_surfdata_at_point(self, indir, file, user_mods_dir, specify_fsurf_out):
        ...
        # extract gridcell closest to plon/plat
        f_tmp = f_in.sel(lsmlon=self.plon.get(360), lsmlat=self.plat, method="nearest")
        ...
        f_out["lsmlon"] = np.atleast_1d(self.plon.get(360))
        f_out["lsmlat"] = np.atleast_1d(self.plat)
        f_out["LATIXY"][:, :] = self.plat
        f_out["LONGXY"][:, :] = self.plon.get(360)
    ...
    def extract_datm_at(self, file_in, file_out):
        ...
        # extract gridcell closest to plon/plat
        f_out = f_in.sel(lon=self.plon.get(360), lat=self.plat, method="nearest")
    ...
    def write_shell_commands(self, file, datm_syr, datm_eyr):
        ...
        with open(file, "w") as nl_file:
            ...
            self.write_to_file("./xmlchange PTS_LON={}".format(self.plon.get(360)), nl_file)
    ...
    def create_datm_at_point(self, datm_tuple: DatmFiles, datm_syr, datm_eyr, datm_streams_file):
        ...
        """
        logger.info("----------------------------------------------------------------------")
        logger.info("Creating DATM files at %s, %s", self.plon.get(360), str(self.plat))
