...
|---|----------|-------------|--------|
| 1 | **Moderate** | No compression step for empty bins — all 16 columns always written, with placeholder values where Swenson would remove the column. `downhill_column_index` assumes all bins populated. | Low risk at OSBS (empty bins unlikely). Add compression step if deploying to other sites. |
| 2 | **Moderate** | `bedrock_depth = 1e6` vs Swenson's 0 — both placeholders, but CTSM may use this to limit soil column depth. | Determine CTSM behavior (STATUS.md #7). |
| 3 | **Low** | Tail removal order differs (after vs before basin masking) — slightly different pixel population for exponential fit. | Minor effect. Consider aligning with Swenson's order for consistency. |
| 4 | **Low** | `_fit_polynomial` dropped Swenson's `weights.size` validation check. | Restore the check (one-line fix). |
| 5 | **Low** | `quadratic()` unguarded for `ak=0` (trap_slope=0) — inherited from Swenson. | Add `ak=0` linear-equation branch. |
| 6 | **Low** | `fit_trapezoidal_width()` unguarded for `n_hillslopes=0` — division by zero. | Add `n_hillslopes <= 0` guard (unlikely to trigger). |
| 7 | **Low** | `max(trap_width, 1)` floor clips any fitted width below 1m — absent in Swenson. | Acceptable at OSBS (sub-meter width indicates bad fit). Document. |
| 8 | **Low** | `circular_mean_aspect()` on empty array produces silent NaN — inherited from Swenson. | Add empty guard if empty bins become possible. |
| 9 | **Info** | `AREA` variable written in NetCDF — Swenson does not. | Verify whether CTSM reads this variable from hillslope file. |
