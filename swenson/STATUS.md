...
| Outlier cutoffs (raw HAND) | Q01 = -6.35 m, Q99 = +17.02 m (true discard; production c260505, post-E.6) | 2026-05-02 | Phase E.5 |
...
| Lake hill_area | Σ(water_mask × pixel_area) = 11.08 km² raw NWI total (production c260505, post-E.6); rescaled per-rep to ~0.021 km²/rep | Phase E.5 | Phase E.5 |
...
issue). A cold-start accelerated-AD spinup (`osbs.swenson.neon.spinup`) then **CONVERGED**
(2026-08-17): ~180 continuous sim-years, 20-yr block-mean drift <1% on all carbon pools.
With both mechanical and scientific sniff tests passing, the formal 200+200/post-AD run was
**deemed unnecessary** and the forcing was **handed to the PI**; the config recipe is delivered
at `docs/neon-forcing-case-recipe.md` (Phase I I7). `osbs.swenson.spinup` 600-yr accelerated
...
| NEON forcing case recipe (Phase I I7) | `swenson/docs/neon-forcing-case-recipe.md` | Drop-in list of forcing-unique case settings (compset, PTS, streams, `dtlimit=-1`, MPILIB) — the PI hand-off |
| MERIT regression | `scripts/merit_validation/merit_regression.py` | Pysheds-fork validation test |

## Change log

- **2026-08-26** — **Audit F1 remediation: corrected stale pre-E.6 values in the local current-state record.** The 2026-08-26 audit (`audit/260826-phase_e_through_i_audit/`) found the "Scientific decisions (locked)" table and several docs still carried pre-E.6 snapshots that the operative production file (`hillslopes_osbs_production_c260505.nc`) supersedes. Corrected to production truth (NetCDF attrs / `production_summary.txt`), anchored to `c260505`: **Q99 raw-HAND cutoff 17.46 → 17.02 m**, **lake `hill_area` 10.68 → 11.08 km²** (11,082,394 px — the ~0.4 km² is the E.6 `binary_fill_holes` gain), **lake perimeter / ½-width 102,810 / 51,405 → 96,540 / 48,270 m**, lake area-fraction 11.8 → 12.3 %; Q01 aligned -6.34 → -6.35 m (rounding). Current-state spots fixed in `STATUS.md`, `phases/{G,H,E.5}-*.md`, `docs/lake-column-ctsm-audit.md`, and two `scripts/` comments/docstrings (both `hillslope_params.py` copies kept byte-identical). Dated historical Log entries left verbatim (true when written); the E.5 paper methods-template was annotated so the stale figures aren't reused. Public `hpg-esm-docs` (6 hits) is a separate later batch. Docs/comments only — no pipeline logic, no data regeneration; the production NetCDF is the authority and was not modified.
